...
async function fetchMenu() {
    try {
        ...
        if (container) {
            container.innerHTML =
                "<p>Unable to load menu. Please make sure the backend is running.</p>";
        }
    }
}
...
function renderMenu() {
    ...
    items.forEach(item => {
        ...
        div.innerHTML = `
            <div class="emoji">${item.image_emoji || "🍽️"}</div>
            <h3>${item.name}</h3>
...
async function checkout() {
    ...
    try {
        ...
        alert(
            `Order placed! Order ID: #${result.id}\n` +
        ...
        const ordersLink =
            document.querySelector('a[data-view="orders"]');
